...
from networks.net import MobileFaceNet
...
def get_loss(net_output, ground_truth):
    ...
    focalloss_eye=MultiClassFocalLossWithAlpha(alpha=[0.2, 0.2, 0.3, 0.1]).to(device)

    #focalloss_eye=nn.CrossEntropyLoss()
...
def validate(model, dataloader,  iteration, device, checkpoint=None):
    ...
    print("Validation  loss: {:.4f}, blurness: {:.4f}, Eye_acc: {:.4f},  mouth_acc: {:.4f}\n".format(
...
if __name__ == '__main__':
    ...
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=512,shuffle=True, num_workers=0,drop_last=True)
    val_dataset = ClassifyDataset(root_path,val_data_file)
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=512,shuffle=True, num_workers=0,drop_last=True)
    ...
    for epoch in range(1, 100 + 1):
        ...
        print("epoch {:4d}, loss: {:.4f},blurness: {:.4f} ,Eye_acc: {:.4f}, mouth_acc: {:.4f}".format(
            epoch,
